...
function dlTest (done) {
  ...
  interval = setInterval(function () {
    ...
    if (!graceTimeDone){
      ...
      dlStatus = ((speed * 8 * settings.overheadCompensationFactor)/1024).toFixed(2) // speed is multiplied by 8 to go from bytes to bits, overhead compensation is applied, then everything is divided by 1048576 to go to megabits/s
...
function ulTest (done) {
  ...
  interval = setInterval(function () {
    ...
    if (!graceTimeDone){
      ...
      var speed = totLoaded / (t / 1000.0)
      ulStatus = ((speed * 8 * settings.overheadCompensationFactor)/1024).toFixed(2) // speed is multiplied by 8 to go from bytes to bits, overhead compensation is applied, then everything is divided by 1048576 to go to megabits/s
